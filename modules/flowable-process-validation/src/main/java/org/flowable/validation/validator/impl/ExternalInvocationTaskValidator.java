...
public abstract class ExternalInvocationTaskValidator extends ProcessLevelValidator
{
  ...
  protected void validateFieldDeclarationsForEmail(org.flowable.bpmn.model.Process process,
      TaskWithFieldExtensions task, List<FieldExtension> fieldExtensions, List<ValidationError> errors)
  {
    boolean toDefined = false;
    boolean textOrHtmlOrTemplateDefined = false;
    ...
    for (FieldExtension fieldExtension : fieldExtensions)
    {
      ...
      {
        textOrHtmlOrTemplateDefined = true;
      }
      ...
      List<ExtensionAttribute> extAttrs = task.getAttributes().get("mailtasktemplateid");
      if (extAttrs != null && extAttrs.size() == 1 && extAttrs.get(0) != null && extAttrs.get(0).getValue() != null)
      {
        textOrHtmlOrTemplateDefined = true;
      }
    }
    ...
    if (!textOrHtmlOrTemplateDefined)
    {
      addError(errors, Problems.MAIL_TASK_NO_CONTENT, process, task,
          "Text, html, textVar, htmlVar field or the mailtasktemplateId attribute should be provided");
    }
  }

  protected void validateFieldDeclarationsForShell(org.flowable.bpmn.model.Process process,
      TaskWithFieldExtensions task, List<FieldExtension> fieldExtensions, List<ValidationError> errors)
  {
    ...
    for (FieldExtension fieldExtension : fieldExtensions)
    {
      ...
      if ((fieldName.equals("wait") || fieldName.equals("redirectError") || fieldName.equals("cleanEnv")) && !fieldValue
          .toLowerCase().equals("true") && !fieldValue.toLowerCase().equals("false"))
      {
  ...
  protected void validateFieldDeclarationsForDmn(org.flowable.bpmn.model.Process process, TaskWithFieldExtensions task,
  ...
  protected void validateFieldDeclarationsForHttp(org.flowable.bpmn.model.Process process, TaskWithFieldExtensions task,
      List<FieldExtension> fieldExtensions, List<ValidationError> errors)
  {
    ...
    for (FieldExtension fieldExtension : fieldExtensions)
    {
      ...
      if (fieldName.equals("requestUrl") && ((fieldValue != null && fieldValue.length() > 0) || (fieldExpression != null
          && fieldExpression.length() > 0)))
      {
    ...
    {
      addError(errors, Problems.HTTP_TASK_NO_REQUEST_METHOD, process, task,
          "No request method is defined on the http activity");
    }

    if (!requestUrlDefined)
    {
      addError(errors, Problems.HTTP_TASK_NO_REQUEST_URL, process, task,
          "No request url is defined on the http activity");
    }
